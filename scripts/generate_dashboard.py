...
def fetch_codeql_prs(token: str, owner: str, repo: str) -> list[dict[str, Any]]:
    """Fetch pull requests that match the Devin fix PR naming convention.

    PRs are identified by looking for ``fix(`` or ``CQLF-`` in the title.
    Both open and closed PRs are fetched (up to 100 each) so the dashboard
    can show merged / outstanding / closed-without-merge counts.
    """
    ...
    for state in ("open", "closed"):
        ...
        try:
            ...
            if resp.status_code == 200:
                for pr in resp.json():
                    title = pr.get("title", "")
                    # Match PRs generated by dispatch_devin.py prompt instructions
                    if "fix(" in title.lower() or "cqlf-" in title.upper():
